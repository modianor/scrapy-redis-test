        ...
        try:
            ...
            if response.url.endswith('page=1'):
                ...
                if all_page:
                    ...
                    for page_num in range(2, all_page + 1):
                        ...
                        next_task = Task(spider_name=self.name, task_type='Detail', url=page_url, filter=True)
                        yield Request(next_task, self.parse, dont_filter=task.filter, meta=response.meta)
            ...
            for tweet_node in tweet_nodes:
                try:
                    ...
                    self.logger.error(e)
            kibanalog = f'name:{self.name} callback:{sys._getframe(0).f_code.co_name} {task.task_type}生成任务成功'
            task.kibanalog = kibanalog
            yield task
        except:
            kibanalog = f'name:{self.name} callback:{sys._getframe(0).f_code.co_name} exception:\n{traceback.format_exc()}'
            self.logger.error(kibanalog)
            task.kibanalog = kibanalog
            yield task
